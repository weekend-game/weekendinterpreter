...
public class Act {
	...
	public Act(WeekendTextEditor app, Editor editor, Filer filer, LastFiles lastFiles, Finder finder, Replacer replacer,
		...
		run = getActRun(filer, runner);
		stop = getActStop(runner);
	...
	public void setRunMode(boolean arg) {
		if (arg) {
			...
			saveOldValue = save.isEnabled();
			save.setEnabled(false);
			saveAs.setEnabled(false);

			cutOldValue = cut.isEnabled();
			cut.setEnabled(false);
			pasteOldValue = paste.isEnabled();
			...
			save.setEnabled(saveOldValue);
			saveAs.setEnabled(true);

			cut.setEnabled(cutOldValue);
			paste.setEnabled(pasteOldValue);
	...
	private AbstractAction getActRun(Filer filer, Runner runner) {
		return new AbstractAction() {
			{
				putValue(Action.NAME, Loc.get("run"));
				putValue(Action.SHORT_DESCRIPTION, Loc.get("run_program"));
				putValue(Action.SMALL_ICON, getImageIcon("run.gif"));
	...
	private AbstractAction getActStop(Runner runner) {
		return new AbstractAction() {
			{
				putValue(Action.NAME, Loc.get("stop"));
				putValue(Action.SHORT_DESCRIPTION, Loc.get("stop_program"));
				putValue(Action.SMALL_ICON, getImageIcon("stop.gif"));
